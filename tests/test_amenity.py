...
class TestAmenity(unittest.TestCase):
    """
    Implements test cases for the Amenity class.
    """

    def setUp(self) -> None:
        """
        Initializes objects of the Amenity class.
        """
        amenities = {
            "one": "Television", "two": "Wifi", "three": "Smart Doors"
            }
        self.amenity1 = Amenity()
        self.amenity2 = Amenity(**amenities)
    ...
    def tearDown(self) -> None:
        ...
        key1 = f"{self.amenity1.__class__.__name__}.{self.amenity1.id}"
        key2 = f"{self.amenity2.__class__.__name__}.{self.amenity2.id}"
    ...
    def test_attributes_present(self):
        """
        Tests that the Amenity object has all required attributes.
        """
        ...
        self.assertIn("one", self.amenity2.__dict__)
        self.assertIn("two", self.amenity2.__dict__)
        self.assertIn("three", self.amenity2.__dict__)
    ...
    def test_str_method(self):
        ...
        representation of an Amenity object.
        """
        return_value = (
            f"[{self.amenity1.__class__.__name__}]"
            f"({self.amenity1.id})({self.amenity1.__dict__})"
        )
        self.assertEqual(str(self.amenity1), return_value)

    def test_save_method(self):
        """
        Tests that the save() method serializes and stores the Amenity object.
        """
        self.amenity1.save()
        key = f"{self.amenity1.__class__.__name__}.{self.amenity1.id}"
    ...
    def test_to_dict_method(self):
        ...
        """
        obj_dict = deepcopy(self.amenity1.__dict__)
        obj_dict["__class__"] = self.amenity1.__class__.__name__
        obj_dict["created_at"] = self.amenity1.created_at.isoformat()
        obj_dict["updated_at"] = self.amenity1.updated_at.isoformat()
        self.assertEqual(self.amenity1.to_dict(), obj_dict)
